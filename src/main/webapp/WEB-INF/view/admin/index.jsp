                        ...
                        <div id="published-posts" class="activity-block"><h3>最近发布</h3> <br>
                            <ul>
                                <c:forEach items="${articleList}" var="article" begin="0" end="4">
                                    <li><span><fmt:formatDate value="${article.article_create_time}"
                                                              pattern="HH:mm MM月dd日"/> </span>
                                        <a href="/SSMBlog/home/article/${article.article_id}">${article.article_title}</a>
                                    </li>
                                </c:forEach>
                            ...
                            <ul id="the-comment-list" data-wp-lists="list:comment">
                                <c:forEach items="${commentList}" var="comment" varStatus="status">
                                    <li class="comment   thread-even comment-item approved">
                                        ...
                                        <img alt="" src="${comment.comment_author_avator}"
                                        ...
                                        <div class="dashboard-comment-wrap has-row-actions">
                                            <p class="comment-meta">
                                                ...
                                                <a target="_blank" href="${comment.comment_author_url}"
                                                   ...
                                                   class="url">${comment.comment_author_name}</a>
                                            </cite>发表在
                                                《<a href="/SSMBlog/home/article/${comment.comment_article_id}">${commentArticleList[status.count-1].article_title}</a>》
                                            </p>
                                            <blockquote><p>${comment.comment_content}</p></blockquote>
                                            <p class="row-actions">
                                                <span class=""> 
                                                <a href="/SSMBlog/admin/comment/edit/${comment.comment_id}">编辑</a>
                                            </span>
                                                <span class=""> |
                                                <a href="javascript:void(0)"
                                                 onclick="deleteComment(${comment.comment_id})">删除</a>
                                            </span>
                                            </p>
                                        </div>
                                    </li>
            ...
            <div id="dashboard_quick_press" class="postbox ">
                <div class="inside">
                    <form name="post" method="post" id="insertDraftForm"
                          class="initial-form hide-if-no-js" action="/SSMBlog/admin/article/insertDraftSubmit">

                        <div class="layui-form-item">
                            <div class="layui-input-block">
                                <input type="text" name="article_title" id="articleTitle" required  lay-verify="required" placeholder="请输入标题" autocomplete="off" class="layui-input">
                            </div>
                        </div>
                        <div class="layui-form-item layui-form-text">
                            <div class="layui-input-block">
                                <textarea name="article_content" placeholder="请输入内容" id="articleContent" class="layui-textarea" required></textarea>
                            </div>
                        </div>
                        <input type="hidden" name="article_status" value="0">
                        <div class="layui-form-item">
                            <div class="layui-input-block">
                    ...
                    <div class="drafts"><p class="view-all"><a
                            href="/SSMBlog/admin/article/index"
                            aria-label="查看所有草稿">查看所有</a></p>
                        <h2 class="hide-if-no-js">草稿</h2>
                        <ul>
                            <c:forEach items="${articleList}" var="article" begin="0" end="4">
                                <c:if test="${article.article_status==0}">
                                    <li>
                                        <div class="draft-title"><a
                                                href="/SSMBlog/admin/article/edit/${article.article_id}">${article.article_title}</a>
                                            <time ><fmt:formatDate value="${article.article_create_time}" pattern="yyyy年MM月dd日"/></time>
                                        </div>
                                    </li>
                                </c:if>
                            </c:forEach>
