...
$("#sina_btn").click(function () {
    ...
    layer.open({
        ...
        content: $('#sina_code'),
            ...
            $("#sina_code").attr("style", "display:none;")
        }
    });
});
...
$("#feed_btn").click(function () {
    ...
    layer.open({
        ...
        content: $('#feed_code'),
            ...
            $("#feed_code").attr("style", "display:none;")
        }
    });
});
...
(function ($) {
    ...
    function write() {
        var content = "<div class=\"ds-dialog\" id='PAY_" + id + "'>"
            ...
            + "       <img  class=\"qrcode-img qrCode_0\" id=\"qrCode_0\" src='/SSMBlog/resource/img/logo.png'  />"
            + "      </div>"
            + "      <p class=\"qrcode-tip\">打赏</p>"
            + "     </div>"
            + "     <div class=\"qrcode-img qrCode_1\" id=\"qrCode_1\">"
            + "      <div class=\"qrcode-border box-size\" style=\"border: 9.02px solid rgb(102, 153, 204\">"
            + "       <img  class=\"qrcode-img qrCode_1\" id=\"qrCode_1\"  src=\"/SSMBlog/resource/img/logo.png\"  />"
            + "      </div>"
            + "      <p class=\"qrcode-tip\">打赏</p>"
            + "     </div>"
            + "     <div class=\"qrcode-img qrCode_2\" id=\"qrCode_2\">"
            + "      <div class=\"qrcode-border box-size\" style=\"border: 9.02px solid rgb(235, 95, 1\">"
            + "       <img  class=\"qrcode-img qrCode_2\" id=\"qrCode_2\"  src=\"/SSMBlog/resource/img/logo.png\"  />"
            + "      </div>"
            + "      <p class=\"qrcode-tip\">打赏</p>"
            + "     </div>"
            + "     </div>"
...
function deleteComment(id) {
    if (confirmDelete() == true) {
        $.ajax({
            async: false,
            type: "POST",
            url: '/SSMBlog/admin/comment/delete/' + id,
            contentType: "application/x-www-form-urlencoded; charset=utf-8",
            dataType: "text",
            complete: function () {
                window.location.reload();
...
function increaseViewCount(articleId) {
    if ($.cookie("viewId") != articleId || $.cookie("viewId") == null) {
        $.ajax({
            async: false,
            type: "POST",
            url: "/SSMBlog/home/article/view/" + articleId,
            dataType: 'text',
...
function increaseLikeCount() {
    if ($.cookie("likeId") != articleId || $.cookie("likeId") == null) {
        $.ajax({
            async: false,
            type: "POST",
            url: "/SSMBlog/home/article/like/" + articleId,
            dataType: 'text',
            contentType: 'application/text',
            success: function (data) {
            	//修改点赞的数据
...
$("#applyLinkForm").submit(function () {
    $.ajax({
        async: false,
        type: "POST",
        url: '/SSMBlog/home/applyLinkSubmit',
        contentType: "application/x-www-form-urlencoded; charset=utf-8",
